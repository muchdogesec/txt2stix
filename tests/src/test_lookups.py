...
mock_extractor = Extractor(
    "ex1", dct=dict(file="mock_terms.txt", stix_mapping="mapping", slug="test_slug")
)
...
def test_load_lookup_from_file():
    with patch(
        "txt2stix.lookups.Path.read_text", return_value="term1\nterm2\nterm3"
    ) as mock_read:
        mock_extractor.terms = None
        result = load_lookup(mock_extractor)
...
def test_find_all():
    mock_extractor.terms = {"term1", "term2"}
    with patch(
        "txt2stix.lookups.find_get_indexes_re", return_value=[5, 15]
    ) as mock_find:
        result = find_all(mock_extractor, mock_input_str, start_id=0)
        assert len(result) == 2
        assert {extraction["value"] for extraction in result.values()} == {
            "term1",
            "term2",
        }
...
def test_merge_lookups():
    extractor1 = make_extractor(
        file="file1.txt",
        stix_mapping="mapping1",
        slug="slug1",
        terms={"term1", "term2"},
    )
    extractor2 = make_extractor(
        file="file2.txt",
        stix_mapping="mapping2",
        slug="slug2",
        terms={"term3", "term4"},
    )
    ...
    assert {r for r in result} == {
        ("term1", extractor1.stix_mapping, extractor1.slug),
        ("term2", extractor1.stix_mapping, extractor1.slug),
        ("term3", extractor2.stix_mapping, extractor2.slug),
        ("term4", extractor2.stix_mapping, extractor2.slug),
    }
...
def test_extract_all():
    ...
    assert {tuple(r["start_index"]) for r in result} == {
        (38,),
        (
            58,
            83,
...
def test_multiple_terms_in_string():
    ...
    assert {(r["value"], tuple(r["start_index"])) for r in result} == {
        ("term1", (0, 12, 36)),
        ("term2", (18, 6)),
    }
