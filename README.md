...
* `--pap_level` (`clear`, `green`, `amber`, `red`, `white`, optional): Assigns the corresponding [official OASIS PAP (Permissible Actions Protocol)](https://github.com/oasis-open/cti-stix-common-objects/tree/main/extension-definition-specifications/pap-marking-definition-f8d) Marking Definition to the report and its report-specific objects (same logic as the Admiralty markings above). `white` is a legacy value retained for older MISP compatibility; prefer `clear` for new use. If omitted, no PAP marking is assigned. See [this blog post](https://www.dogesec.com/blog/what_is_pap_why_does_it_matter_cti/) for more on PAP.
...
txt2stix always detects the language of the input using [`py3langid`](https://github.com/adbar/py3langid) and sets the `lang` property on the Report object it generates. If `--ai_content_check_provider` is set, the AI model is also asked to identify the language of the document, and its value takes priority over the `py3langid` result when both are available.
