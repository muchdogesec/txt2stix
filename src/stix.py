import enum
import logging
from stix2 import Indicator, Report, Identity, Note, MarkingDefinition, Relationship, Bundle
from stix2.parsing import dict_to_stix2, parse as parse_stix
from stix2.serialization import serialize
import codecs, base64
import hashlib
from stix2 import NetworkTraffic, IPv4Address, IPv6Address, Indicator, File, DomainName, URL, Directory, EmailAddress, WindowsRegistryKey, AutonomousSystem
import requests
...
class TLP_LEVEL(enum.Enum):
    ...
    @classmethod
    def levels(cls):
        return dict(clear=cls.CLEAR, green=cls.GREEN, amber=cls.AMBER, amber_strict=cls.AMBER_STRICT, red=cls.RED)

    @classmethod
    def values(cls):
        return [cls.CLEAR.value, cls.GREEN.value, cls.AMBER.value, cls.AMBER_STRICT.value, cls.RED.value]
...
class txt2stixBundle:
    ...
    def __init__(self, name, identity, tlp_level, description, confidence, extractors, labels, job_id=None) -> None:
        ...
        self.report = Report(
            ...
            object_refs=[f"note--{self.uuid}"], #won't allow creation with empty object_refs
            object_marking_refs=[
                self.tlp_level.value.id
            ],
            labels=labels,
            published=dt.now(),
            external_references=[
    ...
    def set_defaults(self):
        ...
        self.bundle.objects.extend(
            [self.default_marking, self.identity, self.report])
    ...
    def add_indicator(self, extracted_dict, add_standard_relationship):
        ...
        indicator["id"] = dict_to_stix2(indicator).id

        if extracted_dict.get("indexes"):
            indicator["external_references"].append(
                dict(source_name="indexes", description=json.dumps(extracted_dict["indexes"])))
        indicators, indicator, relation_objects = build_observables(self, stix_mapping, indicator, extracted_dict, extractor)
        if not indicators:
            raise MinorExcption(f"build observable returns {indicators} from extraction: {extracted_dict}")
        self.id_map[extracted_id] = relation_objects
        ...
        if add_standard_relationship and not indicator:
            for r in relation_objects:
                self.add_standard_relationship(r["id"], self.report.id, "extracted-from")
        elif add_standard_relationship:
            self.add_standard_relationship(indicators[0].id, self.report.id, "extracted-from")

    def add_ai_relationship(self, gpt_out):
        if gpt_out['source_ref'] in self.whitelisted_refs or gpt_out['target_ref'] in self.whitelisted_refs:
            return
        
        for source_ref in self.id_map.get(gpt_out['source_ref']):
            for target_ref in self.id_map.get(gpt_out['target_ref']):
                self.add_standard_relationship(source_ref['id'], target_ref['id'], gpt_out['relationship_type'])
    ...
    def process_observables(self, observables, add_standard_relationship=True):
        for key, ex in observables['extractions'].items():
            try:
                ex["id"] = key
                self.add_indicator(ex, add_standard_relationship)
            except BaseException as e:
                logger.exception(f"ran into exception while processing observable `{ex}`", stack_info=True)

    def process_relationships(self, observables):
        for key, relationship in observables.items():
            try:
                relationship['id'] = key
                self.add_ai_relationship(relationship)
            except BaseException as e:
                logger.exception(f"ran into exception while processing relationship `{relationship}`", stack_info=True)
