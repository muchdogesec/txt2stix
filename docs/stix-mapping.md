...
* `indicator`
* `process`
* `relationship` (`indicator` -> `process`)
...
```json
{
    ...
    "name": "Process: <EXTRACTED OBSERVABLE VALUE>",
    "pattern_type": "stix",
    "pattern": "[ process:command_line = '<EXTRACTED OBSERVABLE VALUE>'",
    "valid_from": "<REPORT CREATED PROPERTY VALUE>",
    "object_marking_refs": [
        "marking-definition--<TLP LEVEL SET>",
        "marking-definition--f92e15d9-6afc-5ae2-bb3e-85a1fd83a3b5"
...
```json
{
    ...
    "spec_version": "2.1",
    "id": "process--<UUIDV5>",
    "command_line": "<EXTRACTED OBSERVABLE VALUE>",
}
```

To ensure duplicate `process` objects are not created for the same values, a UUIDv5 address is generated for the ID as follows;
